...
class RNN():
    def __init__(self, mtype='LSTM'): #, learning_rate = 0.001
    ...
    def _build(self, mtype='LSTM'):

        #### THE MODEL THAT WILL BE TRAINED
        rnn_x = Input(shape=(None, Z_DIM + ACTION_DIM + 1))
        
        if mtype == 'LSTM' or mtype == 'lstm':
            lstm = LSTM(HIDDEN_UNITS, return_sequences=True, return_state = True)
        elif mtype == 'GRU' or mtype == 'gru':
            lstm = GRU(HIDDEN_UNITS, go_backwards=True, return_sequences=True, return_state = True)
